...
const (
	tagKindStream = backup.KindTagPrefix + backup.StreamKind
	tagKindVolume = backup.KindTagPrefix + backup.VolumeKind
...
// The container runs `tail -f /dev/null` so the labels are observable to
// conba's discovery for the duration of the test. Reused across every
...
// passes a restic environment block. conba execs the pre-backup command
// inside the target container via the Docker SDK and streams its stdout
// into `restic backup --stdin`; restic refuses to start without a cache
// directory, so we set both PATH (so the runtime can reach docker) and
...
func TestPreBackup_AlongsideMode_ProducesStreamAndVolumeSnapshots(t *testing.T) {
	...
	// The volume snapshot must carry kind=volume, symmetric to the
	// kind=stream tag on the stream snapshot. This is the discriminator
	// that lets downstream tooling tell volume snapshots apart from
	// stream snapshots that share the same container= identity.
	if !slices.Contains(volumeSnaps[0].Tags, tagKindVolume) {
		t.Fatalf(
			"alongside mode: volume snapshot must carry %s; tags=%v",
			tagKindVolume, volumeSnaps[0].Tags,
		)
	}
...
// TestPreBackup_PartialOutputThenNonZeroExit_StoresNoSnapshot is the
// load-bearing integrity guard for the feature's hard correctness rule:
// a pre-backup command that emits partial stdout and THEN exits non-zero
// must abort the target and write NO snapshot. conba withholds EOF from
// restic until the command's exit status is known and, on a non-zero
// exit, terminates restic (context cancel) before it can finalize a
// snapshot of the truncated stream.
// The cycle must exit non-zero and the container's stream-snapshot count
// must not increase despite the partial bytes restic already consumed.
func TestPreBackup_PartialOutputThenNonZeroExit_StoresNoSnapshot(t *testing.T) {
	containerName := uniqueName(t, "conba-e2e-pb-partial")
	volumeName := containerName + "-data"
	...
	startPreBackupContainer(t, preBackupContainerOpts{
		...
		VolumeName: volumeName,
		Labels: map[string]string{
			// Emit partial output, then fail. If conba did not honor the
			// non-zero exit, restic would commit a snapshot of the partial
			// stdout, silently producing a corrupt/truncated backup.
			"conba.pre-backup.command": "echo partial; exit 1",
		},
	})

	cfg, repoPath := preBackupSetup(t, []string{"^" + containerName + "$"}, true)

	// Count stream snapshots before the run. A fresh repo has none, so the
	// baseline is the integrity reference: the count must be unchanged after
	// the failed command.
	...
	if len(after) != len(before) {
		t.Fatalf(
			"partial-output failure must not store any stream snapshot for %s: "+
				"count went from %d to %d (tags=%v)",
			containerName, len(before), len(after), allTags(after),
		)
	}

	requireStdoutContains(t, backupResult, "Failed "+containerName+" stream")
}
...
// exactly one snapshot tagged kind=stream in snaps.
