...
// optsFor builds the backup.Options literal used throughout this file's
// tests, keeping each call site readable while satisfying exhaustruct.
func optsFor(
	backupFn backup.Func,
	streamFn backup.StreamFunc,
	...
	return backup.Options{
		...
		StreamFn:         streamFn,
		Execer:           noopExecer{},
		PreBackupEnabled: preBackupEnabled,
		Hostname:         hostname,
	}
}
...
func recordingStreamFn(err error) (backup.StreamFunc, *[]string) {
	...
	return func(_ context.Context, filename string, _ []string, stdin io.Reader) error {
...
func TestRun_AllSucceed(t *testing.T) {
	...
	backupFn, _ := stubBackupFn(nil, nil)
...
func TestRun_AllFail(t *testing.T) {
	...
	backupFn, _ := stubBackupFn(errBackup, errBackup)
...
func TestRun_PartialFailure(t *testing.T) {
	...
	backupFn, _ := stubBackupFn(nil, errBackup)
...
func TestRun_PassesVolumeTags(t *testing.T) {
	t.Parallel()
	...
	expected := backup.BuildVolumeTags(target, "testhost")
...
func TestRun_SourceUnreadableIsSkipped(t *testing.T) {
	...
	backupFn, _ := stubBackupFn(wrapped)
...
func TestRun_MixedSkipAndFail(t *testing.T) {
	...
	wrapped := fmt.Errorf("wrapped: %w", restic.ErrSourceUnreadable)
	backupFn, _ := stubBackupFn(nil, wrapped, errBackup)
...
func TestRun_AllSkipped(t *testing.T) {
	...
	backupFn, _ := stubBackupFn(wrappedA, wrappedB)
