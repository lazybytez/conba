...
// TestPreBackup_ContainerLabelIgnored asserts that the removed
// conba.pre-backup.container label has no effect: pre-backup commands always
// run in the labeled container, so a stray container label is parsed as just
// another unrecognised label and the resulting spec is unaffected.
func TestPreBackup_ContainerLabelIgnored(t *testing.T) {
	t.Parallel()

	target := makePreBackupTarget(map[string]string{
		filter.LabelPreBackupCommand: "mysqldump --all-databases",
		"conba.pre-backup.container": "some-sidecar",
	})
