...
func TestNewRestoreCommand_AllFlagsExist(t *testing.T) {
	...
	for _, name := range boolFlags {
		...
		if flag.DefValue != wantBoolDefault {
			t.Errorf("flag %q default = %q, want %q", name, flag.DefValue, wantBoolDefault)
		}
	}
}
...
func TestRestoreCore_StreamMode_NoPath(t *testing.T) {
	t.Parallel()
	...
	snap := streamSnapshot()
	snap.Paths = nil
	deps.snapshotsResult = []restic.Snapshot{snap}
	...
	err := cli.RunRestoreCore(context.Background(), opts, deps)
	if err == nil {
		t.Fatal("want error for stream snapshot with no recorded path, got nil")
	}

	if !strings.Contains(err.Error(), "no recorded path") {
...
func TestRestoreCore_StreamMode_HappyPathLive(t *testing.T) {
	...
	wantCmd := []string{"sh", "-c", "psql -U postgres"}
	if !reflect.DeepEqual(deps.execCmd, wantCmd) {
		t.Errorf("exec cmd = %v, want %v", deps.execCmd, wantCmd)
	}
}
...
// ContainerRunning implements cli.RestoreCoreDeps via the ContainerRuntime side.
...
// Exec implements cli.RestoreCoreDeps via the ContainerRuntime side.
func (s *restoreStubs) Exec(_ context.Context, name string, cmd []string, stdin io.Reader) error {
