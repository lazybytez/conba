...
func TestPrintDryRun_SingleTarget(t *testing.T) {
	...
	err := cli.PrintDryRun(&buf, targets, true)
	if err != nil {
...
func TestPrintDryRun_Empty(t *testing.T) {
	...
	err := cli.PrintDryRun(&buf, nil, true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPrintPreBackupSummary_LabeledContainer(t *testing.T) {
	...
	wantPrefix := "pre-backup: db mode=replace filename=db"
...
func TestPrintPreBackupSummary_ResolvesFilenameOverride(t *testing.T) {
	t.Parallel()
	...
	want := "pre-backup: mysql mode=alongside filename=mysql.sql"
...
// TestPrintDryRunWithPreBackup_DisabledIgnoresLabels asserts that with the
// feature disabled, a container carrying pre-backup labels renders the
// legacy volume listing only -- no "would run" or "would skip" lines -- so
// the dry-run reflects what the actual backup would do (back the volume up
// normally), rather than promising a stream that will never run.
func TestPrintDryRunWithPreBackup_DisabledIgnoresLabels(t *testing.T) {
	t.Parallel()
	...
	err := cli.PrintDryRun(&buf, targets, false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if strings.Contains(output, "would run:") {
		t.Errorf("disabled feature must not emit a 'would run:' line; got:\n%s", output)
	}

	if strings.Contains(output, "would skip:") {
		t.Errorf("disabled feature must not emit a 'would skip:' line; got:\n%s", output)
	}

	if !strings.Contains(output, "/var/lib/docker/volumes/pgdata/_data") {
		t.Errorf(
			"disabled feature should emit the legacy volume listing; got:\n%s",
			output,
		)
	}
}
