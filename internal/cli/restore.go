...
var (
	...
	errStreamSnapshotNoPath = errors.New(
		"stream snapshot has no recorded path; cannot determine dump filename",
	)
)
...
type RestoreCoreDeps interface {
	...
	// Exec runs cmd inside the named container with stdin attached, via the
	// Docker SDK exec (the runtime CommandExecer).
...
func runStreamRestore(
	...
	// A conba-written stream snapshot always records exactly one path (the
	// --stdin-filename). Guard against a malformed or foreign snapshot that
	// carries the kind=stream tag without one, so the CLI returns a clean
	// error instead of panicking on the index.
	if len(snap.Paths) == 0 {
		return fmt.Errorf("stream mode: %w", errStreamSnapshotNoPath)
	}
	...
	// RestoreCoreDeps already provides ContainerRunning + Exec with the
	// signatures restore.ContainerRuntime needs, so deps satisfies it
	// directly -- no adapter required.
	err = restore.RunStream(ctx, runOpts, deps.Dump, deps)
	if err != nil {
...
// behind RestoreCoreDeps. Exec runs the in-container command through the
// Docker SDK (the runtime CommandExecer) -- the same path the backup
// pre-backup feature uses -- so no docker CLI binary is required.
...
// through the Docker SDK exec (the runtime CommandExecer), the same path
// the backup pre-backup feature uses. The command's stdout is passed
// through to the operator's terminal; a non-zero exit yields an error.
func (p *productionRestoreDeps) Exec(
	ctx context.Context, name string, cmd []string, stdin io.Reader,
) error {
	err := p.runtime.Exec(ctx, name, cmd, stdin, os.Stdout)
	if err != nil {
