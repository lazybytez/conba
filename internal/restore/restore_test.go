...
// fakeRuntime is an in-memory ContainerRuntime double. Fields are populated
...
func (f *fakeRuntime) Exec(_ context.Context, name string, cmd []string, stdin io.Reader) error {
	f.execCalled = true
	f.execName = name
	f.execCmd = cmd
...
func TestRunStream_LivePath_BuildsArgvAndPipesPayload(t *testing.T) {
	...
	wantCmd := []string{"sh", "-c", "mysql -uroot"}
	if !reflect.DeepEqual(runtime.execCmd, wantCmd) {
		t.Errorf("cmd = %v, want %v", runtime.execCmd, wantCmd)
	}
